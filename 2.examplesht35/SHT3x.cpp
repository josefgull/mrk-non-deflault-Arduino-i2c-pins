...
uint8_t SDA2 = 8;
	uint8_t SCL2 = 9;
	TwoWire I2Ctwo = TwoWire(&sercom1, SDA2, SCL2);
...
void	SHT3x::Begin()
{
	I2Ctwo.begin();               // SDA pin 21, SCL pin 22 TTGO TQ
  	pinPeripheral(8, PIO_SERCOM); //Assign SDA function to pin 0
...
void	SHT3x::UpdateData()
{
	...
	if ((_LastUpdateMillisec == 0) || ((millis()-_LastUpdateMillisec)>=_UpdateIntervalMillisec))
	{
		...
		if (_Error == noError)
		{
			I2Ctwo.requestFrom(_Address, (uint8_t)6);
			uint32_t WaitingBeginTime = millis();
			while ((I2Ctwo.available()<6) && ((millis() - WaitingBeginTime) < _TimeoutMillisec))
			{
...
void 	SHT3x::SendCommand(uint8_t MSB, uint8_t LSB)
{
	...
	uint8_t success = I2Ctwo.endTransmission();
}
